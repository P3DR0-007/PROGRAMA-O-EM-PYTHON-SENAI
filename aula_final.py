...
def inserir_usuario():
    ...
    if nome and email:
        conn = conectar()
        c = conn.cursor()
        c.execute('INSERT INTO usuarios(id, nome, email, endereço, telefone)  VALUES(?,?,?,?)',(nome, email, endereço, telefone))
        conn.commit()
        conn.close()
...
def editar():
    ...
    if selecao:
        ...
        if novo_nome and novo_email:
            conn = conectar()
            c = conn.cursor()
            c.execute('UPDATE usuarios SET nome = ? , email = ? , telefone = ? , endereço = ? , WHERE id = ?',(novo_nome, novo_email, novo_telefone, novo_endereço, user_id))
            conn.commit()
            conn.close()
...
#---------------------------------------------------------------------------------
label_endereço  = tk.Label(janela, text='endereço: ', font=('arial', 15))
label_endereço.grid(row=1,column=0, padx=10, pady=10)
entry_endereço =  tk.Entry(janela, font=('arial', 15))
entry_endereço.grid(row=1,column=1, padx=10, pady=10)
#---------------------------------------------------------------------------------
entry_telefone =  tk.Entry(janela, font=('arial', 15))
entry_telefone.grid(row=2,column=0, padx=10, pady=10)
label_telefone  = tk.Label(janela, text='telefone: ', font=('arial', 15))
label_telefone.grid(row=2,column=1, padx=10, pady=10)
#---------------------------------------------------------------------------------

label_nome  = tk.Label(janela, text='NOME: ', font=('arial', 15))
label_nome.grid(row=3,column=0, padx=10, pady=10)
...
btn_salvar.grid(row=7,column=2, padx=2, pady=30) 

btn_editar = tk.Button(janela, text = 'Editar', font= ('arial', 15), command=editar)
btn_editar.grid(row=8,column=2, padx=2, pady=30) 

btn_Deletar = tk.Button(janela, text = 'Deletar', font= ('arial', 15), command=delete_usuario)
btn_Deletar.grid(row=9,column=2, padx=2, pady=30) 


columns = ('ID', 'NOME', 'E-MAIL', 'ENDEREÇO', 'TELEFONE')
